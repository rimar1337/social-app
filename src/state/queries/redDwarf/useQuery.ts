import type * as ATPAPI from '@atproto/api'
import {
...
export function constructIdentityQuery(didorhandle?: string) {
  return queryOptions({
    queryKey: ['reddwarf_identity', didorhandle],
    queryFn: async () => {
...
export function constructConstellationQuery(query?: {
  ...
  const constellationHost =
    /*store.get(constellationHostAtom) ??*/ 'constellation.microcosm.blue'
  // : QueryOptions<
...
export function useQueryConstellation(query?: {
  ...
  | undefined {
  //if (!query) return;
  return useQuery(constructConstellationQuery(query))
}
...
export function constructConstellationInfiniteQuery(query?: {
  method:
    | '/links'
    | '/links/distinct-dids'
    | '/links/count'
...
export function constructPreferencesQuery(
  agent?: ATPAPI.AtpAgent | undefined,
  pdsUrl?: string | undefined,
) {
  return queryOptions({
    queryKey: ['reddwarf_preferences', agent?.did],
    queryFn: async () => {
...
export function constructArbitraryQuery(uri?: string) {
  return queryOptions({
    queryKey: ['reddwarf_arbitrary', uri],
    queryFn: async () => {
      if (!uri) return undefined as undefined
      const res = await fetch(
        `https://slingshot.microcosm.blue/xrpc/com.bad-example.repo.getUriRecord?at_uri=${encodeURIComponent(uri)}`,
...
export function constructAuthorFeedQuery(did: string, pdsUrl: string) {
  return infiniteQueryOptions({
    ...
    queryFn: async ({
      ...
      readonly ['reddwarf_authorFeed', string],
      string | undefined
...
export function useInfiniteQueryFeedSkeleton(options: {
  ...
  return useInfiniteQuery({
    ...
    enabled:
      !!options.feedUri &&
      (options.isAuthed
        ? !!options.agent && !!options.pdsUrl && !!options.feedServiceDid
        : true),
  })
}
